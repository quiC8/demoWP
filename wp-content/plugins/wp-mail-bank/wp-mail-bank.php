<?php
/**
 * Plugin Name: WP Mail Bank - Mail, SMTP, Email Logs for WordPress
 * Plugin URI: https://mail-bank.tech-banker.com/
 * Description: Facing issues with emails? Mail Bank SMTP Plugin could be an answer! It sends, logs your outgoing emails, supports gmail smtp, and much more!
 * Author: Tech Banker
...
if ( ! defined( 'MAIL_BANK_VERSION_NUMBER' ) ) {
	define( 'MAIL_BANK_VERSION_NUMBER', '3.0.45' );
}
...
function mail_bank_admin_notice_class() {
	...
	class Mail_Bank_Admin_Notices {
		...
		public function mb_admin_notice_ignore() {
			...
			if ( isset( $_REQUEST['mb_admin_notice_ignore'] ) ) {// Input var okay.
				$admin_notices_option = get_option( 'mb_admin_notice', array() );
				$admin_notices_option[ wp_unslash( $_REQUEST['mb_admin_notice_ignore'] ) ]['dismissed'] = 1;// Input var okay.
				update_option( 'mb_admin_notice', $admin_notices_option );
		...
		public function mb_admin_notice_temp_ignore() {
			...
			if ( isset( $_REQUEST['mb_admin_notice_temp_ignore'] ) ) {// Input var okay.
				$admin_notices_option = get_option( 'mb_admin_notice', array() );
				$current_date         = current_time( 'm/d/Y' );
				$date_array           = explode( '/', $current_date );
				$interval             = ( isset( $_REQUEST['mb_int'] ) ? wp_unslash( $_REQUEST['mb_int'] ) : 7 );// Input var okay.

				$date_array[1] += $interval;
				$new_start      = date( 'm/d/Y', mktime( 0, 0, 0, $date_array[0], $date_array[1], $date_array[2] ) );

				$admin_notices_option[ wp_unslash( $_REQUEST['mb_admin_notice_temp_ignore'] ) ]['start']     = $new_start;// Input var okay.
				$admin_notices_option[ wp_unslash( $_REQUEST['mb_admin_notice_temp_ignore'] ) ]['dismissed'] = 0;// Input var okay.
				update_option( 'mb_admin_notice', $admin_notices_option );
		...
		public function mb_admin_notice_pages( $pages ) {
			foreach ( $pages as $key => $page ) {
				if ( is_array( $page ) ) {
					if ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] === $page[0] && isset( $_REQUEST['tab'] ) && $_REQUEST['tab'] === $page[1] ) {// Input var okay.
						return true;
					}
				} else {
					if ( 'all' === $page ) {
					...
					if ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] === $page ) {// Input var okay.
						return true;
					}
				}
				return false;
...
function add_popup_on_deactivation_mail_bank() {
	...
	class Mail_Bank_Deactivation_Form {
		...
		function post_user_feedback_mail_bank() {
			$mail_bank_deactivation_reason = isset( $_REQUEST['reason'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['reason'] ) ) : ''; // Input var okay.
