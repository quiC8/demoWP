			...
			switch ( $page_url ) {
				...
				case 'mb_email_configuration':
					...
					if ( EMAIL_CONFIGURATION_MAIL_BANK === '1' ) {
						...
						<?php
						if ( isset( $_REQUEST['auto_mail'] ) && esc_attr( wp_unslash( $_REQUEST['auto_mail'] ) ) === 'true' ) {// Input var okay.
							?>
							...
							<?php
						} elseif ( isset( $_REQUEST['auto_mail'] ) && esc_attr( wp_unslash( $_REQUEST['auto_mail'] ) ) === 'false' ) {// Input var okay.
							?>
							jQuery("#ux_div_mail_console").css("display", "none");
							jQuery("#ux_div_test_mail").css("display", "block");
							mail_bank_third_step_settings();
