		...
		foreach ( $blog_ids as $blog_id ) {
			...
			if ( false !== $version ) {
				...
				if ( 'enable' === esc_attr( $settings_remove_tables_unserialize['remove_tables_at_uninstall'] ) ) {
					...
					$wpdb->query( 'DROP TABLE IF EXISTS ' . $wpdb->prefix . 'mail_bank_meta' );// db call ok; no-cache ok.
					$wpdb->query( 'DROP TABLE IF EXISTS ' . $wpdb->prefix . 'mail_bank_email_logs' );// db call ok; no-cache ok.
					// Delete options.
